\chapter{Exercise 28: Intermediate Makefiles 习题28：中级 Makefiles}
...
在接下来的三个习题中你将创建一个骨架项目目录，这个目录将在以后的程序构建中被使用到。在这个习题中我将只讲解 \file{Makefile} 的部分。
...
首先创建一个 \file{c-skeleton} 目录，然后将基本文件集放入其中。以下是我的起始代码：
...
\item[README.md] 项目使用的基本介绍。它以 \file{.md} 结尾，这将被解释为一个断节。
...
\item[src/] 源代码目录。 通常是 \file{.c} 和 \file{.h} 文件。
\item[tests/] 自动测试目录。
\item[src/dbg.h] 为了之后的便利，习题20中的 \file{dbg.h} 被拷贝过来了。
\end{description}
