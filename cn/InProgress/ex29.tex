...
\item[静态] 其实，你在之前的练习中使用\program{ar}和\program{ranlib}创建
    ...
    大一点的\file{.o}文件。
\item[动态] 动态链接库的典型后缀名有\file{.so}，\file{.dll}，在OSX上
...
在之前的练习中我已经向你展现了如何创建静态链接库（\file{.a}文件），并且我将在本书的
剩余部分继续使用。在本章练习中我还会向你展示如何创建一个简单的.so库，如
何通过Unix\program{dlopen}系统动态加载它。我要求你自己动手亲自完成它，这有助
于你理解究竟发生了什么，而在接下来的加分练习中则会使用\file{c-skeleton}骨架来创建它。

\subsection{动态加载一个共享库}

为了动态加载一个共享库我将会创建两个源文件。其中一个用来创建一个\file{libex29.so}库，另一个用来创建一个叫\program{ex29}的程序，这个程序能够动态加载这个库，并且使用其中的函数。
...
我们将要做的是使用\ident{dlopen}，\ident{dlsym}和\ident{dlclose}函数与上面的函数协同工作。
...
\item[ex29.c:5] 我会在之后调用库里的函数时用到这个函数指针的类型定义。这不是新知识，不过你还是应该确认一下你是否了解它的作用。
    ＆
\item[ex29.c:18] 如果出现一个系统错误，我会先做一个常规的检查，然后退出，不过请注意，我在结尾的地方使用了\ident{dlerror}来找出与错误相关的库。
\item[ex29.c:20] 我使用\ident{dlsym}函数通过函数在\ident{func\_to\_run}中的\emph{字符串}名，从\ident{lib}中获取函数。这是这个程序中非常犀利的地方，因为我通过命令行\ident{argv}得到的一个字符串，动态地获取了一个函数指针。
...
既然你现在已经知道这个文件的用处了，我准备了一个shell脚本，这个脚本可以用来构建\file{libex29.so}和\program{ex20}，同时在之后还会协同我一起工作。继续往下学习，你会了解怎样手动构建这些东西。
