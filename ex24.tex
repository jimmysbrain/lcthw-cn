...
\item[ex24.c:30-32] 用\func{fgets} 获取姓名中的名字, 这个函数从输入获取一个字符串
    (这里是从 \ident{stdin}) 输入名字的时候不要太长，防止存储名字的缓冲区不够长导致内存溢出。
\item[ex24.c:34-36] 依然用 \func{fgets}来获取\ident{you.last\_name} 。
\item[ex24.c:38-39] 用\func{fscanf} 从\func{stdin} 获取一个整数，并且将这个整数存储到变量
    \ident{you.age}.  会看到用\ident{printf} 打印整数的时候使用了同样的格式表示符。
    为了让 \ident{fscanf} 可以把输入的值存储在某个指针变量指定的位置上，你必须提供把\emph{address} 的地址取出来作为指针变量的值。 
这个例子生动的展示了如何把一串数据的内存地址作为输出参数。
\item[ex24.c:41-45] 通过遍历 \ident{EyeColor} 的枚举数值，来输出眼睛的颜色所具有的所有可能性。
\item[ex24.c:47-50] 继续 \func{fscanf} ，给\ident{you.eyes}赋予数值， 
    同时确保这个数值是有真实有效的。 这很重要，因为有的人会输入一个不在\ident{EYE\_COLOR\_NAMES}数组范围之内的数字，从而导致一个段错误。
...
到目前一切顺利，但是这个练习的关键是 \func{scanf} 是怎么被输入搞垮的。
. 当处理简单的数字转换的时候还好，但是当处理字符串的时候就完全的垮掉了，
因为读取动作完成以前告诉 \func{scanf}输入缓冲区有多大非常困难。函数 \func{gets}
...
就能看到仅靠\func{fscanf} 和字符串就能如何搞垮代码。

注意到程序似乎读入了太多而且似乎吃掉了你的回车键么？这东西似乎没有做你希望做的，
and really
