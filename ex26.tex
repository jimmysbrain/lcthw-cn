...
If you are on more recent versions of Ubuntu and you get errors about \ident{apr\_off\_t} or \ident{off64\_t} then add \verb|-D_LARGEFILE64_SOURCE=1| to \ident{CFLAGS}.

Another thing is you need to add \file{/usr/local/apr/lib} to a file in
/etc/ld.conf.so.d/ then run \program{ldconfig} so that it picks up
...
\begin{description}
\item[bstrlib.o] Comes from \file{bstrlib.c} and header file \file{bstlib.h} which
    you already have.
